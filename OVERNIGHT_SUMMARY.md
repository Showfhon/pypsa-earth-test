...
> 最後更新 2026-09-10 02:30。**10 節點的無 CCL 與 CCL 兩版皆已解出**（見第六之二節）。
> 20 節點 + CCL 執行中，跑完自動接 30 節點 + CCL。
...
求解方面：校準後從 10 節點重建基準線，**無 CCL 42 分鐘 / CCL 63 分鐘皆收斂**。
先前 30 節點在未校準需求下不收斂的結論**不能當成求解器極限**（見第五節但書），
20 / 30 節點正在校準後的需求下重測。
...
## 六之二、10 節點結果：無 CCL（對照組） vs CCL（研究設計）

兩者都在 10 節點 / 3H / 全年 / reserve off / IPM(tol 1e-4) 下解出，
碳上限同為 1.499e8 tCO2 且**兩者都完全綁定**。

| 執行 | IPM 迭代 | 時間 | 目標函數 |
|---|---:|---:|---:|
| 無 CCL | 71 | 42 分 | **1.8842e10 EUR** |
| CCL | 82 | 63 分 | **2.2803e10 EUR** |

> **在同一個碳上限下，執行官方 2036 容量規劃比自由最小成本解貴 21.0%。**
> 兩者碳排都剛好用滿 149.9 Mt、碳價幾乎相同（26.895 vs 26.896 EUR/t），
> 所以這 21% 純粹是容量配置被鎖定的代價，不是減碳目標不同造成的。

### CCL 約束的落點 —— 模型自己說出張力在哪

| carrier | CCL 下界 | CCL 上界 | 模型解 | 落點 |
|---|---:|---:|---:|---|
| solar | 59,130 | 72,270 | **72,270** | **貼上界**（模型想蓋更多） |
| wind 合計 | 30,680 | 37,498 | **30,680** | **貼下界**（模型被逼著蓋） |
| CCGT | 64,600 | 64,600 | 64,600 | 釘死 |
| nuclear | 31,700 | 31,700 | 31,700 | 釘死 |
| biomass | 1,620 | 1,620 | 1,620 | 釘死 |

wind 合計 = onwind 29,148 + offwind-ac 1,204 + offwind-dc 327。
方向性與無 CCL 版一致（自選 solar 116 GW / wind 4.2 GW）：
**官方規劃的風電目標高於最小成本所願，太陽能上限則卡住了模型。**

### 完整對照

| 指標 | 無 CCL | CCL | 論文 |
|---|---:|---:|---:|
| 總系統成本 | 1.884e10 | 2.280e10 (+21.0%) | — |
| solar | 115,949 MW | 72,270 MW | 65,700 |
| wind 合計 | 4,185 MW | 30,680 MW | 34,089 |
| CCGT | 44,372 MW | 64,600 MW | 64,600 |
| nuclear | 31,161 MW | 31,700 MW | 31,700 |
| biomass | 11,348 MW | 1,620 MW | 1,620 |
| coal 發電 | 126.2 TWh | 83.3 TWh | — |
| CCGT 發電 | 90.8 TWh | 210.5 TWh | — |
| nuclear 發電 | 207.5 TWh | 220.3 TWh | — |
| **battery p_nom_opt** | 6.9 MW | **2.9 MW** | **30,000 MW** |
| ESS 放電量 | 1.27 TWh | 1.23 TWh（1.21 為既有水力） | — |
| **AC 線路擴建** | +1.55% | **+1.70%** | **+32%** |
| **VRE 棄電率** | 0.85% | **0.43%** | **2.9%** |
| 碳排 / 上限 | 149.9 / 149.9 Mt | 149.9 / 149.9 Mt | — |
| 碳價 | 26.895 EUR/t | 26.896 EUR/t | — |
| load shedding | 0.00 TWh | 0.00 TWh | — |

### ⚠️ 三個與論文的系統性落差（列為 20 / 30 節點的觀察項）

| 指標 | 我們（10 節點 3H） | 論文（120 節點 1H） |
|---|---:|---:|
| battery | 2.9 MW | 30,000 MW |
| AC 擴建 | +1.7% | +32% |
| VRE 棄電率 | 0.43% | 2.9% |

三者彼此一致，都指向**空間與時間解析度不足**：
節點少 → 沒有輸電壅塞 → 不需擴線；時段 3 小時 → 日內波動被平滑 →
沒有套利空間 → 不需儲能 → 也不會棄電。

**驗證方法**：若節點數上去後這三個數字同步上升，就證實是解析度造成的，
而非模型設定錯誤。20 / 30 節點跑完要回頭比對這張表。

### 其他觀察

- **onwind 容量因數 0.230 → 0.170**：被迫從 2 GW 蓋到 29 GW，只能往較差的風場擴，資源品質下降
- **CCGT 從備用轉主力**：發電 90.8 → 210.5 TWh（CF 0.234 → 0.372），取代燃煤與生質能
- **Model status "Unknown" 不是失敗**：`ipm_optimality_tolerance: 1e-4` 的直接後果
  （P-D 相對誤差 3.03e-05 / 1.97e-05 超過 HiGHS 內部的 1e-7 門檻）。
  解本身可行且品質良好（primal residual ~1e-06、dual residual ~1e-10）

---

## 七、目前的執行計畫（已排定）

1. ~~10 節點 / 3H / 無 CCL~~ ✅ **已解出**：71 次迭代 / 42 分 / 1.884e10
2. ~~10 節點 / 3H / + CCL~~ ✅ **已解出**：82 次迭代 / 63 分 / 2.280e10
   （CCL 並未拖慢收斂，迭代數只多 15%）
3. **20 節點 + CCL** —— 執行中（`chain_20_30.sh`，timeout 6h）
4. **30 節點 + CCL** —— 已串接在 20 節點之後（timeout 8h）。
   30 節點先前的失敗全在 2.25 倍需求下，校準後尚未真正測過
